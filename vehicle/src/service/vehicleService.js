...
const assignOrUnAssignDriver = async (vehicleIdNumber, driverId, action) => {
    try {
        if (action == rsInfo.UNASSIGN) {
          const deallocateDriver = await vehicle.findOneAndUpdate({VIN:vehicleIdNumber},{$unset:{userId: " "}},{new:true});
            ...
            if (valid_vehicle) {
                if (valid_vehicle.vehicleStatus == rsInfo.VSTATUS) {
                    ...
                    if (dProfile.availabilityStatus == rsInfo.DRIVER_STATUS) {
                        const assignDriver = await vehicle.findOneAndUpdate({ VIN: vehicleIdNumber }, { $set:  { userId: driverId } });
